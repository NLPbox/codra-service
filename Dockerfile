...
RUN apt-get update -y && \
    apt-get install -y python3 python3-pip graphviz graphviz-dev libxml2-dev libxslt-dev xvfb && \
    pip3 install hug sh && \
...
RUN pip3 install pudb # TODO: rm after debug
RUN apt-get install -y imagemagick python-tk

WORKDIR /opt/codra_service
ADD convert.sh dis2png.py codra_hug_api.py /opt/codra_service/
EXPOSE 8000
